...
def _rows_for_day(entry_day_utc: pd.Timestamp, tz: str = "Europe/Berlin"):
    ...
    entry_time ~ 11:50 local (spec-inspired). We care about:
    ...
    for h in [0, 1, 2]:
        rows.append(
            {
                "entry_time": entry_local,  # local tz; loader will normalize to UTC
                ...
                "post_delivery_feature": 888.0,     # leakage
            }
        )
    return rows
...
def test_datasource_whitelist_and_end_now(tmp_path):
    # build synthetic CSV with two entry days, 3 hours each
    ...
    # All displayed entry_time values are <= end_now
    ...
    # We expect exactly 3 hours for the day, in [0..23]
